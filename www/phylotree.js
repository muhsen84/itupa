 ...
 */
  d3.layout.phylotree = function(container) {
    var item_selected = d3_phylotree_item_selected,
      ...
      is_node_collget_newickapsed = d3_phylotree_is_node_collapsed,
    ...
    var self = {},
      ...
      def_branch_length_accessor = function(_node) {
        ...
        //modify_selection ("No branch length for ", _node.name);
        return undefined;
      },
    ...
    (draw_branch = draw_line), (draw_scale_bar = null), (rescale_node_span = 1),
    (count_listener_handler = function() {}), (layout_listener_handler = function() {}),
    (node_styler = undefined), (edge_styler = undefined), (shown_font_size = font_size),
    (selection_attribute_name = "selected"), (popover_displayed = null), (right_most_leaf = 0),
    (label_width = 0), (radial_center = 0), (radius = 1), (radius_pad_for_bubbles = 0),
      ...
      if (phylotree.radial()) {
        ...
        nodes.forEach(function(d) {
          ...
          if(options["layout"] == "right-to-left"){
            d.y = _extents[1][1]*scales[1] - d.y + label_width;
          }
 ...
 */
    ...
    phylotree.handle_node_click = function(node) {
      ...
      /*menu_object
          ...
          .append("a")
          .attr("tabindex", "-1")
          .text("Find Corrsponding Subtree")
          .on("click", function(d) {
            menu_object.style("display", "none");
            find_corresponding_subtree(node, tree_name);
          });*/
        ...
        if (!d3_phylotree_is_leafnode(node)) {
          ...
          if (options["selectable"]) {
            menu_object
              ...
              .on("click", function(d) {
                menu_object.style("display", "none");
                phylotree.modify_selection (function (d) { return false;}).placenodes().update();
 ...
 *
 * what label is written (optional).
 * @returns {String} tree's name.
 */
    phylotree.get_tree_name = function() {
      return tree_name;
    };

    phylotree.set_tree_name = function(new_name) {
      tree_name = new_name;
    };
 ...
 */
 let first_time_transform = true;
    phylotree.update = function(transitions) {
	  ...
	  let drag_tree = d3.behavior.drag()
		.on("dragstart", function (d){
		})
		.on("drag", function(d,i) {
			let max_height =  svg.attr("height");
			let max_width =  svg.attr("width");
			let box = d3.select(this).node().getBBox();
			
			let x = 0, y = 0;
			if (this.getAttribute("transform") != null){
				let transform = this.getAttribute("transform").replace("translate", "")
	...
	enclosure.call(drag_tree);

	if (first_time_transform) {
      ...
      if(options["zoom"]) {
        var zoom = d3.behavior.zoom()
          ...
          .on("zoom", function() {
            ...
            if (tree_name == "tree1") {
              d3.select("#tree1_viz").select("." + css_classes["tree-container"])
                  .attr("transform", "translate(" + translate + ")scale(" + d3.event.scale + ")");
            }else{
              d3.select("#tree2_viz").select("." + css_classes["tree-container"])
                  .attr("transform", "translate(" + translate + ")scale(" + d3.event.scale + ")");
            }
          });
        svg.call(zoom);
 ...
 */
    ...
    phylotree.draw_edge = function(container, edge, transition) {
      ...
      if (transition) {
        ...
        container.transition().duration(1).attr("d", new_branch_path);
      } else {
      ...
      if (d3_phylotree_is_leafnode(node)) {
        ...
        labels
          .on("mouseover", function(d) {
            if (phylotree.mouseOverCorrspondingSubtree){
              find_corresponding_subtree(d, tree_name);
            }
          })
        ...
        if (phylotree.radial()) {
          ...
          (transitions && options["layout"] == "left-to-right" ? labels.transition() : labels)
            .attr("text-anchor", "start")
            .attr("transform", function(d) {
              if(options["layout"] == "right-to-left"){
                let node_width = getTextWidth(node_label(d), shown_font_size + "px sans-serif");
                return phylotree.align_tips() ? d3_phylotree_svg_translate([-10, 0]):
                    d3_phylotree_svg_translate([phylotree.shift_tip(d)[0] - node_width - 10, 0]);
              }
          ...
          circles
            .on("mouseover", function(d) {
              ...
              .attr("style", function(d){
                if (d.bcn && phylotree.showColorCorrespondingSubtree && !has_hidden_nodes(d)) {
                  var color1 = d3.scale.linear()
                      .domain([0, 1])
                      .range(['#000000', '#00ff00'])
                      .interpolate(d3.interpolateHcl);

                  return "fill: " + color1(d.bcn) + ";";
                }
              })
            .attr("r", function(d) {
              if (d.bcn && !has_hidden_nodes(d)){
                let min_r = 5, max_r = 10;
                if(phylotree.correspondingSubtreeNodesRadius != undefined){
                  min_r = parseInt(phylotree.correspondingSubtreeNodesRadius[0]);
                  max_r = parseInt(phylotree.correspondingSubtreeNodesRadius[1]);
                }
                return (max_r - min_r)*(d.bcn-0)/(1-0) + min_r; // normalize bcn values in the range [5,10]
              }else {
 ...
 */
  ...
  function d3_phylotree_resize_svg(tree, svg, tr) {
    ...
    if (svg) {
      ...
      svg.attr("height", sizes[1] + 500).attr("width", sizes[0] + 500);
    }
    return sizes;
  }
